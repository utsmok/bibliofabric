...
@pytest.mark.asyncio
async def test_gettable_mixin_get_success(gettable_client, mock_api_client, mock_unwrapper):
    entity_id = "123"
    mock_raw_item = {"id": entity_id, "value": "Test Value"}

    # Mock the response from BaseApiClient.request
    mock_response = MagicMock(spec=httpx.Response) # Ensure it's a spec for httpx.Response
    mock_response.json.return_value = {"results": [mock_raw_item], "header": {"numFound": 1}}
    mock_api_client.request.return_value = mock_response # Should return the response object directly
    ...
    mock_unwrapper.unwrap_results.assert_called_once_with({"results": [mock_raw_item], "header": {"numFound": 1}})
...
@pytest.mark.asyncio
async def test_gettable_mixin_get_not_found(gettable_client, mock_api_client, mock_unwrapper):
    entity_id = "notfound"
    mock_response = MagicMock(spec=httpx.Response)
    mock_response.json.return_value = {"results": [], "header": {"numFound": 0}} # Empty results
    mock_api_client.request.return_value = mock_response
    mock_unwrapper.unwrap_results.return_value = []

    with pytest.raises(BibliofabricError, match=f"MockEntityModel with ID '{entity_id}' not found."):
        await gettable_client.get(entity_id)


@pytest.mark.asyncio
async def test_gettable_mixin_get_no_entity_model(gettable_client, mock_api_client, mock_unwrapper):
    entity_id = "raw123"
...
@pytest.mark.asyncio
async def test_gettable_mixin_get_parsing_error(gettable_client, mock_api_client, mock_unwrapper):
...
@pytest.mark.asyncio
async def test_searchable_mixin_search_success(searchable_client, mock_api_client, mock_unwrapper):
    ...
    result = await searchable_client.search(page=1, page_size=10, sort_by="id asc", filters=filters)
...
@pytest.mark.asyncio
async def test_searchable_mixin_search_no_search_model(searchable_client, mock_api_client):
...
@pytest.mark.asyncio
async def test_searchable_mixin_search_parsing_error(searchable_client, mock_api_client):
...
@pytest.mark.asyncio
async def test_cursor_iterable_mixin_iterate_success(cursor_iterable_client, mock_api_client, mock_unwrapper):
    ...
    results = []
    async for item in cursor_iterable_client.iterate(page_size=1, filters={"active": True}):
        results.append(item)
...
@pytest.mark.asyncio
async def test_cursor_iterable_mixin_iterate_no_entity_model(cursor_iterable_client, mock_api_client, mock_unwrapper):
...
@pytest.mark.asyncio
async def test_cursor_iterable_mixin_iterate_parsing_error(cursor_iterable_client, mock_api_client, mock_unwrapper):
    ...
    results = []
    # Should log warning and yield raw data
    async for item in cursor_iterable_client.iterate():
        results.append(item)
...
@pytest.mark.asyncio
async def test_cursor_iterable_mixin_iterate_empty_initial_results(cursor_iterable_client, mock_api_client, mock_unwrapper):
