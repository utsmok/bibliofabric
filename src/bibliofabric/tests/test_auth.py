...
def test_static_token_auth_init_no_token():
    ...
    with pytest.raises(ConfigurationError, match="StaticTokenAuth requires a non-empty 'token'."):
...
def test_client_credentials_auth_init_missing_params():
    ...
    with pytest.raises(ConfigurationError, match="ClientCredentialsAuth requires 'client_id', 'client_secret', and 'token_url'."):
        ClientCredentialsAuth(client_id=None, client_secret="secret", token_url="http://token.com")
    with pytest.raises(ConfigurationError, match="ClientCredentialsAuth requires 'client_id', 'client_secret', and 'token_url'."):
        ClientCredentialsAuth(client_id="id", client_secret=None, token_url="http://token.com")
    with pytest.raises(ConfigurationError, match="ClientCredentialsAuth requires 'client_id', 'client_secret', and 'token_url'."):
...
@pytest.mark.asyncio
@patch("httpx.AsyncClient.post")
async def test_client_credentials_auth_fetch_token_no_access_token_in_response(mock_post):
...
async def test_client_credentials_auth_fetch_token_http_error(mock_post):
    ...
    mock_post.side_effect = httpx.HTTPStatusError(
        "Bad Request", request=httpx.Request("POST", "http://token.com"), response=httpx.Response(400)
    )
...
async def test_client_credentials_auth_fetch_token_network_error(mock_post):
    ...
    with pytest.raises(AuthError, match="Failed to fetch access token: Network unreachable"):
        await auth._fetch_access_token()
...
async def test_client_credentials_auth_authenticate_fetches_token(mock_post):
    ...
    mock_post.return_value = httpx.Response(
        200, json={"access_token": "new_token"},
        request=httpx.Request("POST", "http://token.com")
    )
