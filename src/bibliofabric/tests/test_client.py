...
@pytest.mark.asyncio
async def test_request_get_method_success_with_model(base_api_client_with_mock_retry, mock_unwrapper):
    """Test .request() for GET with an expected Pydantic model."""
    class DummyModel(BaseModel):
        data: str

    mock_http_response = httpx.Response(200, json={"data": "success"}, request=httpx.Request("GET", "/test_model"))
    ...
    base_api_client_with_mock_retry._request_with_retry.return_value = (mock_http_response, parsed_dummy_model, 1)

    result = await base_api_client_with_mock_retry.request("GET", "/test_model", expected_model=DummyModel)
...
@pytest.mark.asyncio
async def test_request_get_method_no_model(base_api_client_with_mock_retry, mock_unwrapper):
    """Test .request() for GET without an expected model returns raw response."""
    mock_http_response = httpx.Response(200, json={"data": "raw_success"}, request=httpx.Request("GET", "/raw"))
    base_api_client_with_mock_retry._request_with_retry.return_value = (mock_http_response, None, 1) # No parsed_model
...
async def test_request_with_hooks(base_api_client_with_mock_retry):
    ...
    mock_http_response = httpx.Response(200, json={"status": "ok"}, request=httpx.Request("GET", "/hook_test"))
    ...
    client._response_unwrapper.unwrap_results = MagicMock(return_value=[{"status": "ok"}]) # if get_total_results etc. are called
    ...
    fresh_client._http_client.send = AsyncMock(return_value=mock_http_response) # Mock send
    ...
    assert post_args[1] is None # parsed_model will be None as dict.model_validate fails
...
async def test_request_post_method(base_api_client_with_mock_retry, mock_unwrapper):
    ...
    mock_http_response = httpx.Response(200, json={"id": "1", "status": "created"}, request=httpx.Request("POST", "/test"))
    base_api_client_with_mock_retry._request_with_retry = AsyncMock(return_value=(mock_http_response, {"id": "1"}, 1))

    json_payload = {"data": "test_payload"}
    response = await base_api_client_with_mock_retry.request("POST", "/test", json_data=json_payload, expected_model=dict)
...
async def test_request_put_method(base_api_client_with_mock_retry, mock_unwrapper):
    ...
    mock_http_response = httpx.Response(200, json={"id": "1", "status": "updated"}, request=httpx.Request("PUT", "/test/1"))
    base_api_client_with_mock_retry._request_with_retry = AsyncMock(return_value=(mock_http_response, {"id": "1"}, 1))

    json_payload = {"data": "updated_payload"}
    response = await base_api_client_with_mock_retry.request("PUT", "/test/1", json_data=json_payload, expected_model=dict)
...
async def test_request_delete_method(base_api_client_with_mock_retry, mock_unwrapper):
    ...
    mock_http_response = httpx.Response(204, request=httpx.Request("DELETE", "/test/1")) # No JSON body for 204
    base_api_client_with_mock_retry._request_with_retry = AsyncMock(return_value=(mock_http_response, None, 1))
    ...
    assert isinstance(response, httpx.Response) # Should return raw response if no model
    assert response.status_code == 204


@pytest.mark.asyncio
async def test_caching_get_request_cache_miss_and_hit(base_api_client_with_cache, mock_unwrapper):
    """Test caching for GET requests: first call is a miss, second is a hit."""
    class SimpleModel(BaseModel):
        key: str

    mock_http_response = httpx.Response(200, json={"key": "value"}, request=httpx.Request("GET", "/cache_test"))
    ...
    result1 = await base_api_client_with_cache.request("GET", "/cache_test", expected_model=SimpleModel)
    assert isinstance(result1, SimpleModel)
    assert result1.key == "value"
    base_api_client_with_cache._request_with_retry.assert_awaited_once()

    # Second call (should be cache hit)
    result2 = await base_api_client_with_cache.request("GET", "/cache_test", expected_model=SimpleModel)
...
@pytest.mark.asyncio
async def test_caching_disabled_for_non_get_requests(base_api_client_with_cache, mock_unwrapper):
    """Test that caching is not applied for non-GET requests."""
    class SimpleModel(BaseModel):
        key: str

    mock_http_response = httpx.Response(200, json={"key": "value_post"}, request=httpx.Request("POST", "/cache_test_post"))
    ...
    # First POST call
    await base_api_client_with_cache.request("POST", "/cache_test_post", json_data={"data": "payload"}, expected_model=SimpleModel)
...
@pytest.mark.asyncio
async def test_request_with_retry_handles_httpx_request_error(base_api_client, mock_unwrapper):
    """Test _request_with_retry handles httpx.NetworkError and raises NetworkError."""
    # Use httpx.NetworkError for this test path
    base_api_client._http_client.send = AsyncMock(side_effect=httpx.NetworkError("DNS resolution failed", request=httpx.Request("GET", "/test")))
    base_api_client._auth_strategy.async_authenticate = AsyncMock()
    base_api_client._unwrapper = mock_unwrapper

    with pytest.raises(NetworkError, match="Network error for https://api.example.com/test: DNS resolution failed"):
        await base_api_client._request_with_retry("GET", "/test")

@pytest.mark.asyncio
async def test_request_with_retry_handles_generic_httpx_request_error(base_api_client, mock_unwrapper):
    ...
    class OtherRequestError(httpx.RequestError): pass
    base_api_client._http_client.send = AsyncMock(side_effect=OtherRequestError("Some other request error", request=httpx.Request("GET", "/test")))
    base_api_client._auth_strategy.async_authenticate = AsyncMock()
    base_api_client._unwrapper = mock_unwrapper

    with pytest.raises(BibliofabricRequestError, match="HTTP request error for https://api.example.com/test: Some other request error"):
        await base_api_client._request_with_retry("GET", "/test")

@pytest.mark.asyncio
async def test_request_with_retry_handles_httpx_http_status_error(base_api_client, mock_unwrapper):
    """Test _request_with_retry handles httpx.HTTPStatusError and raises BibliofabricRequestError."""
    mock_http_response = httpx.Response(500, text="Server Error", request=httpx.Request("GET", "/test"))
    base_api_client._http_client.send = AsyncMock(side_effect=httpx.HTTPStatusError("Server Error", request=httpx.Request("GET","/test"), response=mock_http_response))
    base_api_client._auth_strategy.async_authenticate = AsyncMock()
    base_api_client._unwrapper = mock_unwrapper

    with pytest.raises(APIError, match="API request failed with status 500"): # Match changed to APIError
        await base_api_client._request_with_retry("GET", "/test")

@pytest.mark.asyncio
async def test_request_with_auth_failure(base_api_client):
    """Test that AuthError during authentication attempt is propagated."""
    base_api_client._auth_strategy.async_authenticate = AsyncMock(side_effect=AuthError("Token expired"))
...
async def test_client_as_context_manager(mock_unwrapper):
    ...
    async with BaseApiClient(base_url="http://example.com", settings=settings, response_unwrapper=mock_unwrapper) as client:
...
@pytest.mark.asyncio
async def test_request_with_base_url_override(base_api_client_with_mock_retry, mock_unwrapper):
    """Test that base_url_override is correctly used in requests."""
    mock_http_response = httpx.Response(200, json={"data": "override_success"}, request=httpx.Request("GET", "https://override.example.com/test"))
    base_api_client_with_mock_retry._request_with_retry.return_value = (mock_http_response, {"data": "override_success"}, 1)

    await base_api_client_with_mock_retry.request("GET", "/test", base_url_override="https://override.example.com", expected_model=dict)
...
@pytest.mark.asyncio
async def test_request_json_alias_for_json_data(base_api_client_with_mock_retry, mock_unwrapper):
    """Test that the 'json' alias for 'json_data' works correctly."""
    mock_http_response = httpx.Response(200, json={"status": "created"}, request=httpx.Request("POST", "/alias_test"))
    base_api_client_with_mock_retry._request_with_retry.return_value = (mock_http_response, {"status": "created"}, 1)

    payload = {"key": "value"}
    # Use the 'json' alias
    await base_api_client_with_mock_retry.request("POST", "/alias_test", json=payload, expected_model=dict)

    args, kwargs = base_api_client_with_mock_retry._request_with_retry.call_args
    assert kwargs['json_data'] == payload # Internally, it should be passed as json_data
...
from bibliofabric.log_config import logger as bibliofabric_logger # Import the specific logger

@pytest.mark.asyncio
async def test_request_both_json_and_json_data_uses_json_data(base_api_client_with_mock_retry, mock_unwrapper):
    """Test that if both 'json' and 'json_data' are provided, 'json_data' is used and a warning is logged."""
    mock_http_response = httpx.Response(200, json={"status": "ok"}, request=httpx.Request("POST", "/conflict_test"))
    base_api_client_with_mock_retry._request_with_retry.return_value = (mock_http_response, {"status": "ok"}, 1)
    ...
    handler_id = bibliofabric_logger.add(log_capture_string, level="WARNING", format="{message}")
    ...
    assert "Both 'json' and 'json_data' provided to request; using 'json_data'." in logged_text
    ...
    if not bibliofabric_logger._core.handlers: # if it's empty after removing our test handler
         from bibliofabric.log_config import configure_logging as bfl_configure_logging # avoid name clash
         bfl_configure_logging() # Re-apply default config


from pydantic import BaseModel
from bibliofabric.exceptions import BibliofabricRequestError, AuthError, NetworkError, APIError
